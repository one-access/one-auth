...
import com.oneaccess.auth.services.auth.ExchangeTokenService;
...
public class AuthenticationController {
    ...
    public ResponseEntity<?> loginUser(@RequestParam(name = "original_request_uri", required = false) String originalRequestUri, @RequestBody LoginRequestDTO loginRequest) {
        ...
        TokenResponseDTO tokenResponseDTO = authenticationService.loginUser(loginRequest);
        if(StringUtils.hasText(originalRequestUri)) {
            tokenResponseDTO.setOriginalRequestUri(originalRequestUri);
        }
     ...
     * Delegates to ExchangeTokenService for the actual exchange logic.
     */
    @PostMapping("/exchange")
    public ResponseEntity<?> exchangeCodeForToken(@Valid @RequestBody TokenExchangeRequestDTO request) {
        log.info("Authentication API: exchangeCodeForToken");
        
        TokenExchangeResponseDTO exchangeResponse = exchangeTokenService.exchangeCodeForToken(request);
        TokenResponseDTO response = TokenResponseDTO.from(exchangeResponse);
